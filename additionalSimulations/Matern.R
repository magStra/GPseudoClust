...
for (k in 1:24){
  print(k)
  simData <- readMat(sprintf("simMatern%d.mat",k))
  PSMs = array(0,dim=c(simData$nGenes,simData$nGenes,24))
  for (j in 1:24)
  {
    A <- as.matrix(read.table(sprintf("simMatern%d_Results_PSM_Chain%d.csv",k,j),sep=","))
    PSMs[,,j] <-  A
  }
...
save(summaryResults,ARI_PY,ARI_DPM,FMI_PY,FMI_DPM,NMI_PY,NMI_DPM,file="MaternNonParametricSummaryPSM.rda")
