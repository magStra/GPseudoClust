...
for j = 1:100
    j
    K = load(sprintf('PSM_simDropout_Mixed_%d.mat',j)); 
    K = K.PSMs;
    clusterSolution = computeSummaryPSM_lmkk(K,2:10);
    csvwrite(sprintf('PSM_simDropoutMixed_lmkk_%d.csv',j),...
        clusterSolution.PSM);
    sumClustSimDropout_llkm(:,j) = clusterSolution.clustering;
end
csvwrite('sumClust_simDropoutMixed_lmkk.csv',...
...
for j = 1:100
    ...
    K = K.PSMs;
    PSM = mean(K,3);
    csvwrite(sprintf('PSM_simDropoutMixed_consensus_%d.csv',j),...
        PSM);
end
...
for j = 1:12
    ...
    K = csvread(sprintf('PSM_simDropoutMixed_lmkk_%d.csv',a(j)));
...
for j = 1:12
     ...
     K = csvread(sprintf('PSM_simDropoutMixed_consensus_%d.csv',a(j)));      
