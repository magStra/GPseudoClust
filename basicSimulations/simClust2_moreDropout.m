...
for j = 1:100
    j
    K = load(sprintf('~/Documents/GPseudoClustCode/GPseudoClust/postProcPSM/dropout/PSM_simDropout30_Subsample_%d.mat',j)); 
    K = K.PSMs;
    clusterSolution = computeSummaryPSM_lmkk(K,2:10);
    csvwrite(sprintf('~/Documents/GPseudoClustCode/GPseudoClust/postProcPSM/dropout/PSM_simDropout30_lmkk_%d.csv',j),...
        clusterSolution.PSM);
  
    sumClustSimDropout30_llkm(:,j) = clusterSolution.clustering;

end
csvwrite('~/Documents/GPseudoClustCode/GPseudoClust/postProcPSM/dropout/sumClust_simDropout30_lmkk.csv',...
     sumClustSimDropout30_llkm);
 
figure()
for j = 1:12
 
     K = csvread(sprintf('~/Documents/GPseudoClustCode/GPseudoClust/postProcPSM/dropout/PSM_simDropout_consensus30_%d.csv',a(j)));      
...
figure()
for j = 1:12
    load(sprintf('~/Documents/GPseudoClustCode/GPseudoClust/postProcPSM/dropout/PSM_simDropout30_DPM_%d.mat',a(j)));
     ax(j) =  subaxis(4,3,j, 'SpacingHoriz', 0.03, 'SpacingVert',0.03,'Padding', 0.01, 'MarginRight', 0.1,...
         'MarginLeft',0.05,'MarginTop',0.01,'MarginBottom',0.05);
    imagesc(weightedPSM);
    colormap(hot);
